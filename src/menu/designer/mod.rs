...
pub fn create_level_pack_overview() -> Rc<RefCell<dyn Container>> {
    ...
    layer.add_component(TextButton::boxed(
        ButtonTextRenderHelper::simple_boxed("Back", font, Region::new(-1.0, 0.6, -0.6, 0.8), BUTTON_COLORS),
        Box::new(|_a, params| {
            params.agent.change_container(create_main_menu());
    })));

    layer.add_component(TextButton::boxed(
        ButtonTextRenderHelper::simple_boxed("New level pack", font, Region::new(-1.0, -0.4, -0.4, -0.2), BUTTON_COLORS),
        Box::new(|_a, params| {
